...
SDL_BackgroundHandler BackgroundHandler = NULL;
void* BackgroundHandlerParam = NULL;

void
SDL_SetBackgroundHandler(SDL_BackgroundHandler callback, void *param)
{
	BackgroundHandler = callback;
	BackgroundHandlerParam = param;
}

static SDL_AppEventHandler AppEventHandler = NULL;
static void* AppEventHandlerParam = NULL;

void
SDL_SetAppEventHandler(SDL_AppEventHandler callback, void *param)
{
	AppEventHandler = callback;
	AppEventHandlerParam = param;
}
...
SDL_SendAppEvent(SDL_EventType eventType)
{
	...
	if (eventType != SDL_APP_LOWMEMORY && AppEventHandler) {
		AppEventHandler(eventType, AppEventHandlerParam);
		return 1;
	}
