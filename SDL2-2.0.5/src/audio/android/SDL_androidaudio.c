...
static jboolean nativeFillAudioPlayout(JNIEnv* env, jarray buffer, SDL_bool is16Bit)
{
	SDL_AudioDevice* this = audioDevice;

	if (didbackground && BackgroundHandler) {
		BackgroundHandler(BackgroundHandlerParam);
	}
	...
	if (now_ticks - last_ticks >= 5000) {
		posix_print("nativeFillAudioPlayout, bufferPinned: %p, length: %i, is16Bit: %s, paused: %s\n", bufferPinned, length, is16Bit? "true": "false", SDL_AtomicGet(&this->paused) ? "true" : "false");
...
JNIEXPORT jboolean JNICALL Java_org_libsdl_app_SDLService_nativeFillByteAudioPlayout(JNIEnv* env, jclass jcls, jarray buffer)
{
	return nativeFillAudioPlayout(env, buffer, SDL_FALSE);
}

JNIEXPORT jboolean JNICALL Java_org_libsdl_app_SDLService_nativeFillShortAudioPlayout(JNIEnv* env, jclass jcls, jarray buffer)
{
	return nativeFillAudioPlayout(env, buffer, SDL_TRUE);
}
