...
static jmethodID midChangeOrientation;
...
JNIEXPORT void JNICALL SDL_Android_Init(JNIEnv* mEnv, jclass cls)
{
	...
	midChangeOrientation = (*mEnv)->GetStaticMethodID(mEnv, mActivityClass,
                                "changeOrientation", "()Z");
    ...
    if (!midGetNativeSurface ||
       !midAudioOpen || !midAudioClose || !midGetAssetType || !midGetDpiScale || !midChangeOrientation || !midBleScanPeripherals || !midBleStopScanPeripherals || !midBleInitialize ||
...
JNIEXPORT void JNICALL Java_org_libsdl_app_SDLActivity_onNativeSurfaceChanged(JNIEnv* env, jclass jcls)
{
	...
	posix_print("onNativeSurfaceChanged, 1, Android_Window: 0x%p, Android_Window->driverdata: 0x%p\n", Android_Window, Android_Window? Android_Window->driverdata: 0);
    ...
    if (data->egl_surface == EGL_NO_SURFACE) {
		posix_print("onNativeSurfaceChanged, 2, data->egl_surface == EGL_NO_SURFACE, data->native_window: 0x%p\n", data->native_window);
	...
	posix_print("onNativeSurfaceChanged, 3, exit\n");
...
void Android_JNI_ChangeOrientation()
{
	struct LocalReferenceHolder refs = LocalReferenceHolder_Setup(__FUNCTION__);
	jmethodID mid;
	JNIEnv* mEnv;
    ...
    (*mEnv)->CallStaticBooleanMethod(mEnv, mActivityClass, midChangeOrientation);
